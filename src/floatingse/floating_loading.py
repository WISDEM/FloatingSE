...
class FloatingFrame(Component):
    ...
    def __init__(self, nFull):
        ...
        # Ballast columns
        self.add_param('ballast_z_full', val=np.zeros((nFull,)), units='m', desc='z-coordinates of section nodes (length = nsection+1)')
        self.add_param('ballast_d_full', val=np.zeros((nFull,)), units='m', desc='outer radius at each section node bottom to top (length = nsection + 1)')
        self.add_param('ballast_t_full', val=np.zeros((nFull,)), units='m', desc='shell wall thickness at each section node bottom to top (length = nsection + 1)')
        ...
        self.add_param('ballast_column_center_of_mass', val=0.0, units='m', desc='z-position of center of column mass')
        self.add_param('ballast_column_Px', np.zeros(nFull), units='N/m', desc='force per unit length in x-direction on ballast')
        self.add_param('ballast_column_Py', np.zeros(nFull), units='N/m', desc='force per unit length in y-direction on ballast')
        self.add_param('ballast_column_Pz', np.zeros(nFull), units='N/m', desc='force per unit length in z-direction on ballast')
        self.add_param('ballast_column_qdyn', np.zeros(nFull), units='N/m**2', desc='dynamic pressure on ballast')
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        R_od_ballast   = 0.5*params['ballast_d_full']
        R_od_tower     = 0.5*params['tower_d_full']

        t_wall_base    = params['base_t_full']
        t_wall_ballast = params['ballast_t_full']
        ...
        z_ballast      = params['ballast_z_full']
        ...
        z_cb_ballast   = params['ballast_column_center_of_buoyancy']
        
        cg_base        = np.r_[0.0, 0.0, params['base_column_center_of_mass']]
        cg_ballast     = np.r_[0.0, 0.0, params['ballast_column_center_of_mass']]
        ...
        Px_ballast, Py_ballast, Pz_ballast = params['ballast_column_Pz'], params['ballast_column_Py'], -params['ballast_column_Px']  # switch to local c.s.
        ...
        npon = len(idx)
        if len(idx) > 0:
            qdyn_pontoon = np.max( np.abs( np.r_[params['base_column_qdyn'], params['ballast_column_qdyn']] ) )
