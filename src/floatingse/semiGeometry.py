...
class SemiGeometry(Component):
    ...
    def __init__(self, nSection):
        ...
        self.add_param('base_section_height', val=np.zeros((nSection,)), units='m', desc='length (height) or each section in the spar bottom to top (length = nsection)')
        ...
        self.add_param('ballast_section_height', val=np.zeros((nSection,)), units='m', desc='length (height) or each section in the spar bottom to top (length = nsection)')
        ...
        self.add_output('base_z_nodes', val=np.zeros((nSection+1,)), units='m', desc='z-coordinates of section nodes (length = nsection+1)')
        self.add_output('ballast_z_nodes', val=np.zeros((nSection+1,)), units='m', desc='z-coordinates of section nodes (length = nsection+1)')
        self.add_output('base_z_section', val=np.zeros((nSection,)), units='m', desc='z-coordinates of section centers of mass (length = nsection)')
        self.add_output('ballast_z_section', val=np.zeros((nSection,)), units='m', desc='z-coordinates of section centers of mass (length = nsection)')
